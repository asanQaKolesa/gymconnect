      ...
      <div className="space-y-3">
        {matchesList.map((item) => (
          <div key={item.id} className="bg-white rounded-[20px] p-4 shadow-[0_4px_16px_rgba(0,0,0,0.03)] border border-black/[0.04] flex items-center justify-between">
            ...
            <button 
              onClick={() => alert(`Открытие чата с ${item.name}`)}
              className="px-4 py-2 bg-[#007AFF] text-white rounded-[12px] text-xs font-semibold shadow-sm active:scale-95 transition-all"
            >
              Написать
            </button>
          </div>
