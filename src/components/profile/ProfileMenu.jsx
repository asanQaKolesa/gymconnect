...
export default function ProfileMenu() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Блок для партнеров и тренеров */}
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-50 text-xs font-semibold text-blue-600 uppercase tracking-wider">
          ...
          {partnerItems.map((item, index) => {
            ...
            return (
              <button 
                key={index}
                onClick={item.action}
                className="w-full px-4 py-3.5 flex items-center justify-between hover:bg-blue-50/40 transition-colors text-left group"
              >
                <div className="flex items-center gap-3">
