...
import { Users, Dumbbell, TrendingUp, LogOut, RefreshCw, X, UserPlus, Ruler, Weight, Calendar, User } from 'lucide-react';
...
                      <span className="font-bold text-slate-800">{selectedStudent.gender || 'Не указан'}</span>
                    </div>
                  </div>
                </div>
...
                  <p className="font-semibold text-slate-900 mb-1">Данные абонемента</p>
                  <p><b>Telegram:</b> @{selectedStudent.username || 'Не указан'}</p>
                  <p><b>Фитнес-зал:</b> {selectedStudent.gym || 'Не указан'}</p>
                  <p><b>Цель:</b> {selectedStudent.goal || 'Не указана'}</p>
...
    </div>
  );
}  полный код напиши  // Функция для красивого отображения целей
...
  if (gender === 'male' || gender === 'М') return 'Мужской';
  if (gender === 'female' || gender === 'Ж') return 'Женский';
  return gender || 'Не указан';
};  всегда новый полный код пиши
